...
from common.proxy import ProxyStatus, ProxyType, ProxyAddParameters
from rpc import rpc
...
Proxies = NewType('Proxies', Dict[str, Account])
...
class Worker:
    ...
    async def update(self,
        ...
        if self.status == ApplicationStatus.READY or self.status == ApplicationStatus.WORK:
            ...
            diff = set(map(lambda x: x['ip'], proxies)) - set(map(lambda x: x.ip, self.proxies))
            ...
            diff = set(map(lambda x: x['login'], accounts)) - set(map(lambda x: x.login, self.accounts))
            ...
            diff = set(map(lambda x: x.login, self.accounts)) - set(map(lambda x: x.login, accounts))
